...
export function useTrackSelection(queue: QueueItem[], settings: Settings) {
  ...
  const { selectedTracks, selectedFeatures, artistSelections } = useSelector(
    (state: RootState) => state.trackSelection
  );
  ...
  const handleInitializeSelection = useCallback(
    (item: QueueItem) => {
  ...
  const handleClearSelectionForInstance = useCallback(
    (instanceKey: string) => {
  ...
  const handleFeatureToggle = useCallback(
    (instanceKey: string, trackKey: string, featuredArtistNames: string[] = [], albumArtistNames: string[] = []) => {
      dispatch(toggleFeature({ instanceKey, trackKey, featuredArtistNames, albumArtistNames }));
    },
    [dispatch]
  );

  const handleArtistToggle = useCallback(
    (instanceKey: string, trackKey: string, artistName: string) => {
      dispatch(toggleArtist({ instanceKey, trackKey, artistName }));
    },
    [dispatch]
  );

  const handleScrobbleModeToggle = useCallback(
    (instanceKey: string, useTrackArtist: boolean) => {
  ...
  const handleToggleParent = useCallback(
    (instanceKey: string, parentIndex: number, subTrackKeys: string[]) => {
      dispatch(toggleParent({ instanceKey, subTrackKeys, parentKey: String(parentIndex) }));
    },
    [dispatch]
  );

  const handleSelectParentAsSingle = useCallback(
    (instanceKey: string, parentKey: string, subTrackKeys: string[]) => {
  ...
  const handleToggleGroup = useCallback(
    (instanceKey: string, groupKeys: string[], parentKeysInGroup: string[]) => {
