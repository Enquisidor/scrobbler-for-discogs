...
export function useQueue(
  ...
  const { queue, scrobbledHistory, isScrobbling, scrobbleError, scrobbleTimeOffset } = useSelector(
  ...
  const addAlbumToQueue = useCallback(
    async (release: DiscogsRelease) => {
      ...
      try {
        const fullRelease = await fetchReleaseTracklist(
          release.id,
  ...
  const removeLastInstanceOfAlbumFromQueue = useCallback(
    (releaseId: number) => {
  ...
  const removeAllInstancesOfAlbumFromQueue = useCallback(
    (releaseId: number) => {
      queue
        .filter((item) => item.id === releaseId)
        .forEach((instance) => clearSelectionForInstance(instance.instanceKey));
      dispatch(removeAllInstancesOf({ releaseId }));
    },
    [queue, clearSelectionForInstance, dispatch]
  );

  const removeAlbumInstanceFromQueue = useCallback(
    (instanceKey: string) => {
  ...
  const handleScrobbleSingleRelease = useCallback(
    async (instanceKey: string) => {
      ...
      try {
        await scrobbleLastfmTracks(
          tracksToScrobble,
          credentials.lastfmSessionKey,
        ...
        clearSelectionForInstance(instanceKey);
        onQueueSuccess(
          `Scrobbled ${tracksToScrobble.length} tracks from "${itemToScrobble.basic_information.title}"!`
        );
      } catch (err) {
