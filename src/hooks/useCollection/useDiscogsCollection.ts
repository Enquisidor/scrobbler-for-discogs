...
export function useDiscogsCollection(
  ...
  const { collection: fullCollection, isLoading, isSyncing, error, isAuthError } = useSelector((state: RootState) => state.collection);
  ...
  useEffect(() => {
    ...
    const worker = async () => {
        while (pagesToFetchRef.current.length > 0) {
            ...
            try {
                const data = await fetchDiscogsPage(credentials.discogsUsername, credentials.discogsAccessToken, credentials.discogsAccessTokenSecret, page, API_SORT, API_SORT_ORDER, PAGE_SIZE);
                if (abortController.signal.aborted) break;
    ...
    const startFullCollectionSync = async () => {
        ...
        try {
            const initialData = await fetchDiscogsPage(credentials.discogsUsername, credentials.discogsAccessToken, credentials.discogsAccessTokenSecret, 1, API_SORT, API_SORT_ORDER, PAGE_SIZE);
