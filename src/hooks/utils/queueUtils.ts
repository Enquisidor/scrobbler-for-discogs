...
export function getReleaseDisplayArtist(
    ...
    if (!metadata || !settings || settings.artistSource === MetadataSourceType.Discogs) {
    ...
    return sourceMeta?.artist || release.basic_information.artist_display_name;
}
...
export function getTrackDisplayArtist(
    ...
    metadata: Record<number, CombinedMetadata> | undefined, 
    settings: Settings, 
...
export function prepareTracksForScrobbling(
    ...
    metadataMap: Record<number, CombinedMetadata>,
    timeOffset: number,
    settings: Settings
): LastfmTrackScrobble[] {
    ...
    const tracksWithInfo = queue.flatMap(release => {
        ...
        return sortedKeys.flatMap(key => {
            ...
            const artistString = finalArtists.length > 0 
                ...
                : getReleaseDisplayArtist(release, metadata, settings);
